...
test.describe("Flujo crítico socio → inscripción → pago → reporte", () => {
  test("permite crear socio, inscribirlo, registrar un pago y ver KPIs", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText("Socio creado correctamente.", { exact: false }),
    ).toBeVisible();
    ...
    const memberId = memberPayload.data.find(
    ...
    await page
      ...
      .fill("Alta generada por Playwright");
    ...
    const dueRow = page
      .getByRole("row", { name: new RegExp(member.documentNumber) })
      .first();
    await expect(dueRow).toBeVisible();
    await dueRow
      .getByRole("button", { name: /Pago manual|Editar pago/ })
      .click();
    ...
    await expect(
      page
        .getByText("No encontramos datos para los filtros seleccionados.")
        .first(),
    ).toHaveCount(0);

    const revenueCard = page
      .locator("article", { hasText: "Ingresos cobrados" })
      .first();
    await expect(revenueCard).toBeVisible();
    const valueText = (
      await revenueCard.locator("p").nth(1).innerText()
    ).trim();
