...
async function debugDuesIssue() {
  ...
  try {
    ...
    for (const member of allMembers) {
      ...
      const [total, paid, pending] = await Promise.all([
        ...
        db
          .select({ count: count() })
          .from(dues)
          .where(and(eq(dues.memberId, member.member.id), eq(dues.status, "PAID"))),
        db
          .select({ count: count() })
          .from(dues)
          .where(and(eq(dues.memberId, member.member.id), eq(dues.status, "PENDING"))),
      ]);
      ...
      if (actualDues !== expectedDues) {
        console.log(
          `   ⚠️  INCONSISTENCIA: Debería tener ${expectedDues} cuotas, tiene ${actualDues}`
        );
    ...
    const enrollmentsWithoutDues = await db
      ...
      .innerJoin(users, eq(members.userId, users.id)).where(sql`NOT EXISTS (
    ...
    if (enrollmentsWithoutDues.length > 0) {
      ...
      for (const item of enrollmentsWithoutDues) {
        console.log(
          `   ${item.member.name} (${item.member.documentNumber}) - Inscripción ${item.enrollment.id}`
        );
      }
    }
  } catch (error) {
